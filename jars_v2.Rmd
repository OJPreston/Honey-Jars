...
jars <- read_csv("jars_20181108.csv")
sites <- read_csv("sites_ll_20181108.csv")
...
honey <-
  ...
  select(c(2,4,5,6,8,9,13,14,15,17,19,20,23,24)) %>% 
  setNames(c("check_id", 
             "check_date", 
             "check_status",
             "name",
...
honey %>% 
  ...
  labs(title = "Relationship between weight of jar and price",
       ...
       x = "Weight (g)",
       y = "Price (£)",
       caption = "\nPositive correlation (R = 0.670, F (1, 31373) = 7.307e+04, p < 0.001)")
...
To start exploring the data in more detail, I'll add *groupings* so it is easier to differentiate between jar weights. I'll use two separate groupings: the first using *manual bins* based on what small, medium and large jar weights might be; and the second bins defined using *statistical similarity (k-means clustering)*. 
